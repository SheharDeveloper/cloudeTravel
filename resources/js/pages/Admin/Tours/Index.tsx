        ...
        <ProtectedRoute>
            ...
            <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '40px 20px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
                    <h1 style={{ fontSize: '28px', fontWeight: 700, color: '#0499ff', margin: 0 }}>Manage Tours</h1>
                ...
                {tours.length === 0 ? (
                    <div style={{ background: '#f9f9f9', padding: '40px', textAlign: 'center', borderRadius: '8px' }}>
                        ...
                        <Link href="/admin/tours/create" style={{ background: '#0499ff', color: '#fff', padding: '10px 20px', borderRadius: '4px', textDecoration: 'none', fontWeight: 700, fontSize: '13px' }}>
                            Create Tour
                        </Link>
                    </div>
                ) : (
                        ...
                        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                            <thead>
                                <tr style={{ background: '#f5f5f5', borderBottom: '2px solid #ddd' }}>
                                    <th style={{ padding: '15px', textAlign: 'left', fontWeight: 700, color: '#0499ff', fontSize: '13px' }}>Tour Name</th>
                                    <th style={{ padding: '15px', textAlign: 'left', fontWeight: 700, color: '#0499ff', fontSize: '13px' }}>Duration</th>
                                    <th style={{ padding: '15px', textAlign: 'left', fontWeight: 700, color: '#0499ff', fontSize: '13px' }}>Location</th>
                                    <th style={{ padding: '15px', textAlign: 'left', fontWeight: 700, color: '#0499ff', fontSize: '13px' }}>Price</th>
                                    <th style={{ padding: '15px', textAlign: 'center', fontWeight: 700, color: '#0499ff', fontSize: '13px' }}>Actions</th>
                                </tr>
                            </thead>
                            <tbody>
                                {tours.map((tour) => (
                                    <tr key={tour.id} style={{ borderBottom: '1px solid #eee' }}>
                                        ...
                                        <td style={{ padding: '15px', fontSize: '13px', color: '#ff6b35', fontWeight: 700 }}>Â£{tour.price || '0'}</td>
                                        <td style={{ padding: '15px', textAlign: 'center', fontSize: '13px' }}>
                                            <Link href={`/admin/tours/${tour.id}/edit`} style={{ color: '#0499ff', textDecoration: 'none', marginRight: '10px', fontWeight: 700 }}>
                                                Edit
                                            </Link>
