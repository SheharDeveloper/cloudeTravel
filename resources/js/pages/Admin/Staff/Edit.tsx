...
export default function StaffEdit() {
    const { staff, zipCodes, addressData, currencies } = usePage().props as any;
    ...
    const [data, setData] = useState({
        ...
        salary_type: payment?.salary_type || 'per_month',
    ...
    const handleNext = async () => {
        ...
        formData.append('staff_uid', staff.uid);
    ...
    const submit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        router.put(`/admin/staff/${staff.uid}`, formData, {
            onSuccess: () => {
                            ...
                            <form onSubmit={submit}>
                                ...
                                {step === 1 && (
                                    <div className="row">
                                        ...
                                        <div className="col-sm-6 mb-4">
                                            <label className="form-label">Password <small className="text-muted">(leave empty to keep current)</small></label>
                                                                    ...
                                                                    <button type="button" className="btn btn-sm btn-danger"
                                                                        onClick={() => {
                                                                            if (confirm('Delete this education record?')) {
                                                                                router.delete(`/admin/staff-educations/${row.id}`, { onSuccess: () => router.reload() });
                                                                            }
                                                                        }}>
                                ...
                                {step === 7 && (
                                    <div>
                                        {staff?.staff_tax_deductions?.length > 0 && (
                                            <div className="mb-4">
                                                <h6 className="mb-3 text-primary">Existing Tax &amp; Deduction</h6>
                                                <table className="table table-sm">
                                                    <thead><tr><th>Name</th><th>Value</th><th>Type</th><th>Action</th></tr></thead>
                                                    <tbody>
                                                        {staff.staff_tax_deductions.map((row: any) => (
                                                            <tr key={row.id}>
                                                                <td>{row.name}</td>
                                                                <td>{row.value}</td>
                                                                <td>
                                                                    <span className={`badge ${row.type === 'tax' ? 'bg-warning' : 'bg-secondary'}`}>
                                                                        {row.type === 'tax' ? 'Tax' : 'Deduction'}
                                                                    </span>
                                                                </td>
                                                                <td>
                                                                    <button type="button" className="btn btn-sm btn-danger"
                                                                        onClick={() => {
                                                                            if (confirm('Delete this record?')) {
                                                                                router.delete(`/admin/staff-tax-deductions/${row.id}`, { onSuccess: () => router.reload() });
                                                                            }
                                                                        }}>
                                                                        <i className="fa fa-trash"></i>
                                                                    </button>
                                        ...
                                        <div className="d-flex justify-content-between align-items-center mb-3">
                                            <h6 className="mb-0">Add Tax &amp; Deduction</h6>
                                            <button type="button" className="btn btn-sm btn-primary"
                                                                        ...
                                                                        <button type="button" className="btn btn-sm btn-danger"
                                                                            onClick={() => {
                                                                                if (confirm('Delete this document?')) {
                                                                                    router.delete(`/admin/staff-documents/${row.id}`, { onSuccess: () => router.reload() });
                                                                                }
                                                                            }}>
                                                                            <i className="fa fa-trash"></i>
                                                                        </button>
                                ...
                                {step === 4 && (
                                    <div className="row">
                                        ...
                                        <div className="col-sm-6 mb-4">
                                            ...
                                            {passport?.front_image && !data.front_image && (
                                        ...
                                        <div className="col-sm-6 mb-4">
                                            ...
                                            {passport?.back_image && !data.back_image && (
